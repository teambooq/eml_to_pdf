  ...
  class Wkhtmltopdf
    ...
    def self.convert(input, output_path)
      Timeout.timeout(EmlToPdf.configuration.timeout, ConversionTimeoutError) do
        IO.popen("#{EmlToPdf.configuration.wkhtmltopdf} --encoding utf-8 --load-error-handling ignore --footer-center [page] --footer-spacing 2.5 --quiet - #{output_path} 2>&1", "r+") do |pipe|
          pipe.puts(input)
          pipe.close_write
